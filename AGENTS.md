...
> Codex lee este archivo automáticamente al inicio de cada sesión.
...
## Code generation constraints for Codex
...
Codex must prefer the smallest possible change to achieve the requested result.
...
Codex must reuse existing names for:
...
Codex must **never** modify the database schema implicitly.
...
- Run Codex inside a sandbox, container, or isolated development environment when possible.
...
model               → exact model used (e.g. gpt-4o, Codex-sonnet-4-6)
...
Este proyecto se construye en sesiones con Codex. Cada sesión:

1. **Codex lee este archivo automáticamente** — contexto completo desde el inicio
...
```
AGENTS.md         → Este archivo — leído automáticamente por Codex
