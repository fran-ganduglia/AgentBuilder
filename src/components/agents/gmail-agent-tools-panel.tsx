...
type AgentToolsResponse = {
  data?: {
    ...
    selectedGmailToolId: string | null;
    selectedGmailIntegrationId: string | null;
    selectedGmailAllowedActions: GmailToolAction[];
    hasDuplicateGmailTools: boolean;
    hasMisalignedGmailTools: boolean;
  };
  error?: string;
};

export function GmailAgentToolsPanel({
  agentId,
  canEdit,
}: {
  agentId: string;
  ...
  const [selectedActions, setSelectedActions] = useState<GmailToolAction[]>([
    ...getDefaultGmailAgentToolConfig().allowed_actions,
  ]);
  ...
  const loadTools = useCallback(async () => {
    ...
    try {
      ...
      setSelectedToolId(result.data.selectedGmailToolId);
      setSelectedIntegrationId(result.data.selectedGmailIntegrationId);
      setHasDuplicateTools(result.data.hasDuplicateGmailTools);
      setHasMisalignedTools(result.data.hasMisalignedGmailTools);

      const tool = result.data.selectedGmailToolId
        ? result.data.tools.find((item) => item.id === result.data?.selectedGmailToolId) ?? null
        : null;

      if (tool) {
        setEnabled(tool.is_enabled ?? true);
        setSelectedActions(
          result.data.selectedGmailAllowedActions.length
            ? result.data.selectedGmailAllowedActions
            : [...getDefaultGmailAgentToolConfig().allowed_actions]
        );
      } else {
        setEnabled(true);
        setSelectedActions([...getDefaultGmailAgentToolConfig().allowed_actions]);
      }
    } catch {
  ...
  async function saveTool() {
    ...
    try {
      ...
      setMessage("Tool Gmail guardada correctamente.");
      await loadTools();
      void router.refresh();
    } catch {
      setError("No se pudo conectar con el servidor.");
  ...
  async function removeTool() {
    ...
    try {
      ...
      setMessage("Tool Gmail eliminada.");
      await loadTools();
      void router.refresh();
    } catch {
      setError("No se pudo conectar con el servidor.");
  ...
  return (
    <section id="agent-tools-gmail" className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="space-y-6 pt-6">
        ...
        {!loading && hasDuplicateTools ? <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">Se detectaron multiples tools Gmail para este agente. La UI usa la configuracion compatible mas reciente.</div> : null}
        {!loading && hasMisalignedTools ? <div className="rounded-lg border border-rose-200 bg-rose-50 p-4 text-sm text-rose-900">Hay tools Gmail historicas apuntando a otra integracion Google. Conviene limpiarlas para evitar diagnosticos confusos.</div> : null}
        ...
        {!loading ? (
          <label className="flex items-center justify-between rounded-lg border border-slate-200 bg-slate-50 px-4 py-3">
            <div>
              ...
              <p className="text-xs text-slate-500">Si queda deshabilitada, el agente no podra usar lectura real ni writes asistidas de Gmail en chat web.</p>
            </div>
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {GMAIL_TOOL_ACTIONS.map((action) => (
                <label key={action} className="flex items-start gap-3 rounded-lg border border-slate-200 bg-white px-4 py-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-slate-900">{getGmailActionLabel(action)}</p>
                    <p className="mt-1 text-xs text-slate-500">{getGmailActionDescription(action)}</p>
                  </div>
                </label>
              ))}
            </div>
