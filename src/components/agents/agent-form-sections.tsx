...
export function AgentBehaviorSection({
  ...
  const isScopeManaged = Boolean(setupState);
  const isPromptOutOfSync =
    isScopeManaged && fields.systemPrompt.trim() !== recommendedPrompt.trim();
  ...
  return (
    <section className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-7">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm leading-relaxed text-slate-600">
            {isScopeManaged
              ? "Este agente usa un prompt compilado server-side desde su scope, capacidades, integraciones y reglas de negocio."
      ...
      {isScopeManaged && setupState ? (
        <div className="mt-6 space-y-4">
          <div className="grid gap-3 sm:grid-cols-3">
            <div className="rounded-2xl border border-sky-200 bg-sky-50 px-4 py-3">
              <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-sky-700">Tipo de agente</p>
              <p className="mt-2 text-sm font-semibold text-slate-900">{AGENT_SCOPE_LABELS[setupState.agentScope]}</p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
              <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-slate-500">Workflow tecnico</p>
              <p className="mt-2 text-sm font-semibold text-slate-900">{setupState.workflowId}</p>
            </div>
            <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3">
              <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-amber-700">Fuera de scope</p>
              <p className="mt-2 text-sm font-semibold text-slate-900">Rechazar y derivar</p>
            </div>
          </div>
          ...
          <div className="rounded-[1.5rem] border border-slate-200 bg-slate-50 p-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-slate-500">Prompt compilado</p>
                <p className="mt-2 text-sm text-slate-600">Vista de solo lectura del prompt recomendado para este agente por scope.</p>
              </div>
              <span className={`rounded-full px-3 py-1 text-[11px] font-bold uppercase tracking-[0.2em] ${
                promptSyncMode === "recommended"
                  ? "bg-emerald-50 text-emerald-700"
                  : "bg-amber-100 text-amber-800"
              }`}>
                {promptSyncMode === "recommended" ? "En sync" : "Desalineado"}
              </span>
            </div>
            <pre className="mt-4 overflow-x-auto whitespace-pre-wrap rounded-2xl border border-slate-200 bg-white p-4 text-sm leading-relaxed text-slate-900">
              {recommendedPrompt}
            </pre>
          </div>
          ...
          {isPromptOutOfSync ? (
            <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3">
              <p className="text-sm font-medium text-amber-900">
                El agente tiene un prompt guardado distinto al compilado actual. Puedes volver al recomendado desde la barra lateral.
              </p>
            </div>
          ) : null}
        </div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
          ...
          <p className="mt-1 text-sm text-slate-600">
            {isScopeManaged
              ? "Ajusta scope, capacidades o instrucciones de negocio antes que editar texto libre del prompt."
              : "Evita meter credenciales, politicas secretas o datos privados en el prompt."}
          </p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
