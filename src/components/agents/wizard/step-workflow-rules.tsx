import {
  AGENT_CAPABILITY_DESCRIPTIONS,
  AGENT_CAPABILITY_LABELS,
  type AgentCapability,
  type PublicWorkflowDefinition,
} from "@/lib/agents/public-workflow";
import type { SuccessMetricId, WorkflowInstanceConfig } from "@/lib/agents/workflow-templates";

const SUCCESS_METRIC_LABELS: Record<SuccessMetricId, string> = {
  conversation_volume: "Volumen de conversaciones",
  messages_processed: "Mensajes procesados",
  actions_executed: "Acciones ejecutadas",
  action_success_rate: "Exito/fallo de acciones",
  human_escalations: "Escalaciones a humano",
  confirmation_requests: "Confirmaciones solicitadas",
  latency_p95: "Latencia promedio y p95",
  integration_incidents: "Incidentes por integracion",
};

type StepWorkflowRulesProps = {
  workflow: PublicWorkflowDefinition;
  capabilities: AgentCapability[];
  businessInstructions: {
    tasks: string;
    restrictions: string;
    handoffCriteria: string;
  };
  instanceConfig: WorkflowInstanceConfig;
  successMetrics: SuccessMetricId[];
  onToggleCapability: (capability: AgentCapability) => void;
  onBusinessInstructionsChange: (
    patch: Partial<{
      tasks: string;
      restrictions: string;
      handoffCriteria: string;
    }>
  ) => void;
  onInstanceConfigChange: (patch: Partial<WorkflowInstanceConfig>) => void;
  onToggleSuccessMetric: (metric: SuccessMetricId) => void;
};

export function StepWorkflowRules({
  workflow,
  capabilities,
  businessInstructions,
  instanceConfig,
  successMetrics,
  onToggleCapability,
  onBusinessInstructionsChange,
  onInstanceConfigChange,
  onToggleSuccessMetric,
}: StepWorkflowRulesProps) {
  return (
    <section className="space-y-8">
      <div>
        <p className="text-sm font-semibold uppercase tracking-[0.2em] text-emerald-700">Paso 4</p>
        <h2 className="mt-2 text-2xl font-bold tracking-tight text-slate-900">
          Capacidades y gobernanza
        </h2>
        <p className="mt-2 max-w-3xl text-sm leading-relaxed text-slate-600">
          Activa solo las capacidades necesarias y define las reglas operativas del agente. Las escrituras sensibles siguen yendo por approvals.
        </p>
      </div>

      <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h3 className="text-sm font-bold uppercase tracking-[0.2em] text-slate-500">Capacidades activas</h3>
            <p className="mt-2 text-sm text-slate-600">
              El workflow unico habilita capacidades amplias. No hay presets de automatizacion visibles.
            </p>
          </div>
          <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-700">
            {workflow.id}
          </span>
        </div>

        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          {workflow.defaultCapabilities.concat([
            "scheduled_jobs",
            "document_generation",
            "integrated_reads",
            "integrated_writes_with_approval",
          ]).filter((value, index, values) => values.indexOf(value) === index).map((capability) => {
            const isActive = capabilities.includes(capability);

            return (
              <label
                key={capability}
                className={`flex cursor-pointer gap-4 rounded-2xl border p-5 transition-all ${
                  isActive
                    ? "border-slate-900 bg-slate-900 text-white shadow-lg shadow-slate-900/10"
                    : "border-slate-200 bg-white text-slate-900 hover:border-slate-300 hover:bg-slate-50"
                }`}
              >
                <input
                  type="checkbox"
                  checked={isActive}
                  onChange={() => onToggleCapability(capability)}
                  className="mt-1 h-4 w-4 rounded border-slate-300 text-slate-900 focus:ring-slate-900"
                />
                <div>
                  <p className="text-base font-bold">{AGENT_CAPABILITY_LABELS[capability]}</p>
                  <p className={`mt-2 text-sm leading-relaxed ${isActive ? "text-slate-200" : "text-slate-600"}`}>
                    {AGENT_CAPABILITY_DESCRIPTIONS[capability]}
                  </p>
                </div>
              </label>
            );
          })}
        </div>
      </article>

      <div className="grid gap-6 xl:grid-cols-2">
        <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <h3 className="text-sm font-bold uppercase tracking-[0.2em] text-slate-500">Reglas operativas</h3>
          <div className="mt-5 space-y-4">
            <div>
              <label htmlFor="business-tasks" className="block text-sm font-semibold text-slate-700">
                Tareas permitidas
              </label>
              <textarea
                id="business-tasks"
                rows={3}
                value={businessInstructions.tasks}
                onChange={(event) => onBusinessInstructionsChange({ tasks: event.target.value })}
                className="mt-2 block w-full resize-none rounded-lg border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 transition-colors hover:bg-white focus:border-slate-900 focus:bg-white focus:outline-none focus:ring-1 focus:ring-slate-900"
              />
            </div>
            <div>
              <label htmlFor="business-restrictions" className="block text-sm font-semibold text-slate-700">
                Restricciones y limites
              </label>
              <textarea
                id="business-restrictions"
                rows={3}
                value={businessInstructions.restrictions}
                onChange={(event) => onBusinessInstructionsChange({ restrictions: event.target.value })}
                className="mt-2 block w-full resize-none rounded-lg border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 transition-colors hover:bg-white focus:border-slate-900 focus:bg-white focus:outline-none focus:ring-1 focus:ring-slate-900"
              />
            </div>
            <div>
              <label htmlFor="business-handoff" className="block text-sm font-semibold text-slate-700">
                Criterios de handoff
              </label>
              <textarea
                id="business-handoff"
                rows={3}
                value={businessInstructions.handoffCriteria}
                onChange={(event) => onBusinessInstructionsChange({ handoffCriteria: event.target.value })}
                className="mt-2 block w-full resize-none rounded-lg border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 transition-colors hover:bg-white focus:border-slate-900 focus:bg-white focus:outline-none focus:ring-1 focus:ring-slate-900"
              />
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                <label htmlFor="schedule-summary" className="block text-sm font-semibold text-slate-700">
                  Horarios / scheduling
                </label>
                <textarea
                  id="schedule-summary"
                  rows={3}
                  value={instanceConfig.scheduleSummary}
                  onChange={(event) => onInstanceConfigChange({ scheduleSummary: event.target.value })}
                  className="mt-2 block w-full resize-none rounded-lg border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 transition-colors hover:bg-white focus:border-slate-900 focus:bg-white focus:outline-none focus:ring-1 focus:ring-slate-900"
                />
              </div>
              <div>
                <label htmlFor="tone-summary" className="block text-sm font-semibold text-slate-700">
                  Estilo de respuesta
                </label>
                <textarea
                  id="tone-summary"
                  rows={3}
                  value={instanceConfig.toneSummary}
                  onChange={(event) => onInstanceConfigChange({ toneSummary: event.target.value })}
                  className="mt-2 block w-full resize-none rounded-lg border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 transition-colors hover:bg-white focus:border-slate-900 focus:bg-white focus:outline-none focus:ring-1 focus:ring-slate-900"
                />
              </div>
            </div>
          </div>
        </article>

        <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <h3 className="text-sm font-bold uppercase tracking-[0.2em] text-slate-500">Metricas observables</h3>
          <p className="mt-2 text-sm text-slate-600">
            Mantén solo métricas realmente medibles en esta fase operativa.
          </p>

          <div className="mt-5 space-y-3">
            {workflow.successMetrics.map((metric) => {
              const isSelected = successMetrics.includes(metric);

              return (
                <label
                  key={metric}
                  className={`flex cursor-pointer gap-3 rounded-2xl border p-4 transition-colors ${
                    isSelected ? "border-slate-900 bg-slate-50" : "border-slate-200 bg-white hover:border-slate-300"
                  }`}
                >
                  <input
                    type="checkbox"
                    checked={isSelected}
                    onChange={() => onToggleSuccessMetric(metric)}
                    className="mt-1 h-4 w-4 rounded border-slate-300 text-slate-900 focus:ring-slate-900"
                  />
                  <span className="text-sm font-semibold text-slate-900">{SUCCESS_METRIC_LABELS[metric]}</span>
                </label>
              );
            })}
          </div>
        </article>
      </div>
    </section>
  );
}
