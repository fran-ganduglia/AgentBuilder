...
type HubSpotConnectionFormProps = {
  initialName: string;
...
function getStatusContainerClass(view: IntegrationOperationalView): string {
  if (view.tone === "rose") return "border-rose-200 bg-rose-50";
  if (view.tone === "amber") return "border-amber-200 bg-amber-50";
...
export function HubSpotConnectionForm({
  initialName,
  integrationId,
  isConnected,
  operationalView,
  ...
  async function handleDisconnect() {
    ...
    try {
      ...
      if (!response.ok || result.error) {
        setError(result.error ?? "No se pudo desconectar HubSpot");
        return;
      }

      setMessage("HubSpot quedo desconectado correctamente.");
      router.refresh();
    } catch {
    ...
    <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      <div className="flex flex-col gap-2 border-b border-slate-100 bg-slate-50 px-7 py-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-orange-50 ring-1 ring-inset ring-orange-600/20">
            <svg className="h-5 w-5 text-orange-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 12h14M12 5l7 7-7 7" />
            </svg>
          </div>
          <div>
            <h2 className="text-base font-bold text-slate-900">HubSpot CRM</h2>
            <p className="mt-0.5 text-sm text-slate-500">
              OAuth org-level real para contactos, empresas, deals, tasks y meetings.
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-600">
              {isConnected ? "La organizacion ya tiene una conexion HubSpot guardada." : "Todavia no hay una conexion HubSpot activa para esta organizacion."}
            </p>
            {hubId ? <p className="mt-2 text-xs font-medium text-slate-500">Hub ID: {hubId}</p> : null}
          </div>
      ...
      <div className="flex flex-col gap-4 border-t border-slate-100 bg-slate-50 px-7 py-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          {integrationId ? (
            ...
            >
              {isDisconnecting ? "Desconectando..." : "Desconectar HubSpot"}
            </button>
          ) : null}
          ...
          >
            {isConnected ? "Reconectar HubSpot" : "Conectar HubSpot"}
          </a>
        </div>
      </div>
    </section>
